...
class PostManager extends ChangeNotifier {
  ...
  Future<PostResult> _postToPlatform(
    ...
    try {
      ...
      if (!service.isContentValid(content)) {
        final result = service.createFailureResult(
          content,
          'Content exceeds character limit of ${service.characterLimit}',
      ...
      // Attempt to publish the post with retry logic
      final result = await service.publishPostWithRetry(content, account);
  ...
  CharacterLimitValidation validateCharacterLimits(
    ...
    final violatingPlatforms = <PlatformType>[];

    for (final platform in selectedPlatforms) {
      final service = _platformServices[platform];
      if (service != null && !service.isContentValid(content)) {
        violatingPlatforms.add(platform);
      }
    }

    if (violatingPlatforms.isNotEmpty) {
      final platformNames = violatingPlatforms
          .map((p) => p.displayName)
          .join(', ');
      final minLimit = violatingPlatforms
          .map((p) => _platformServices[p]?.characterLimit ?? 0)
          .reduce((a, b) => a < b ? a : b);

      return CharacterLimitValidation(
        isValid: false,
        errorMessage:
            'Content ($contentLength characters) exceeds character limit for: $platformNames. '
            'Maximum allowed: $minLimit characters.',
        violatingPlatforms: violatingPlatforms.toSet(),
        contentLength: contentLength,
        minCharacterLimit: minLimit,
      );
    }
